...
start_backend() {
    ...
    if [ ! -d "$PREVIEW_API_DIR" ]; then
        echo "错误: 后端项目目录不存在: $PREVIEW_API_DIR"
        return 1
    fi
    ...
    echo "启动后端服务器..."
    nohup uvicorn app.main:app --host 0.0.0.0 --port 3300 --reload >> "$PREVIEW_API_LOG" 2>&1 &
    PREVIEW_API_PID=$!
    echo $PREVIEW_API_PID > "$PREVIEW_API_PID_FILE"
    
    echo "等待后端服务启动..."
    for i in $(seq 1 15); do
        if curl -s http://localhost:3300/health > /dev/null 2>&1; then
            echo "✅ 后端服务已启动 (PID: $PREVIEW_API_PID)"
