...
### 位置
- 目录路径: `/workspace/vessel-backend`

### 待确认信息
- **技术栈**: 框架（如Node.js/Express, Java/Spring Boot, Python/Django等）
- **数据库**: 数据库类型和连接方式
- **API结构**: 现有的API端点和路由结构
- **项目结构**: 目录组织和文件分布
- **依赖管理**: 包管理工具和主要依赖
...
### API通信
- 前端通过fetch API调用后端接口
- 基础API地址示例: `http://192.168.10.47:3300/api/v1/`
- 建议使用统一的API错误处理和响应格式
...
当 AI 在 `vessel-frontend/` 或 `vessel-backend/` 目录下生成或修改代码后，**必须在回复的最后**输出以下固定格式的标识块，以便前端解析识别。
...
**字段说明**：
- `target_project`: 字符串，可选值：`"vessel-frontend"` | `"vessel-backend"` | `"both"`
- `timestamp`: ISO 8601 格式的时间戳（当前时间）

**触发条件**：
- 在 `vessel-frontend/` 目录下创建、修改或删除任何文件 → 必须输出标识
- 在 `vessel-backend/` 目录下创建、修改或删除任何文件 → 必须输出标识
- 同时修改前后端代码时，`target_project` 设置为 `"both"`
- **即使只修改一个文件，也必须输出标识**
...
3. **第三步**：验证服务启动和端口
   - 确认前端在 http://localhost:5173
   - 确认后端在 http://localhost:3300
   - 验证功能是否正常工作

4. **第四步**：如果有错误，修复后重新执行停止→启动流程
...
**启动脚本说明**
- 启动脚本位置: `/workspace/start-vessel.sh`
- 停止脚本位置  `/workspace/stop-vessel.sh`
- 脚本会自动处理依赖安装和环境配置
- 前后端会在独立的终端窗口中启动
